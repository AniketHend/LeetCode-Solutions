...
struct sparseTable {
    ...
    void init(int x) {
        ...
        table.assign(k + 1, vector<long long>(n + 1, INT_MAX));
    }
    ...
    void build(vector<int>& arr) {
        ...
        for (int j = 1; j <= k; j++) {
            for (int i = 0; i + (1ll << j) <= n; i++) {
                table[j][i] = operation(table[j - 1][i],
                                        table[j - 1][i + (1ll << (j - 1))]);
            }
        }
    }
    long long query(int l, int r) {
        int j = logs[r - l + 1];
        long long answer = operation(table[j][l], table[j][r - (1ll << j) + 1]);
        return answer;
    }
};
...
template <typename node, typename upd> struct LAZY {
    ...
    LAZY(vector<int>& a) {
        ...
        LT.resize(s);
        fill(LT.begin(), LT.end(), false);
        UT.resize(s);
        fill(UT.begin(), UT.end(), upd());
        build(0, n - 1, 1);
    }
    ...
    void apply(int i, int s, int e, upd& u) {
        if (s != e) {
            LT[i] = 1, UT[i].comb(u, s, e);
        }
        u.apply(ST[i], s, e);
    }
    void update(int s, int e, int i, int l, int r, upd& u) {
        if (s > r || e < l)
            return;
        if (s >= l && e <= r) {
            apply(i, s, e, u);
...
template <typename node, typename upd> struct TREE {
    vector<node> ST;
    vector<int> arr;
    int n;
    int s;
    ...
    void update(int s, int e, int i, int qi, upd& u) {
        if (s == e) {
            u.apply(ST[i]);
            return;
        }
        int mid = (s + e) / 2;
        if (mid >= qi) {
            update(s, mid, 2 * i, qi, u);
        } else {
            update(mid + 1, e, 2 * i + 1, qi, u);
        }
        ST[i].merge(ST[2 * i], ST[2 * i + 1]);
    }
    node query(int s, int e, int i, int l, int r) {
        ...
        int mid = (s + e) / 2;
        node ln, rn, ans;
        ln = query(s, mid, 2 * i, l, r);
        rn = query(mid + 1, e, 2 * i + 1, l, r);
        ans.merge(ln, rn);
        return ans;
    }
...
public:
    int minimumDifference(vector<int> nums) {
        ...
        dbg(nums, a, b);
        auto f = [&](vector<ll>& c, vector<set<ll>> &dp) {
            ll M = sz(c);
            for (ll mask = 0; mask < (1 << M); mask++) {
                ...
                dp[popc(mask)].insert(csum);
            }
        };
        vector<set<ll>> dp1(n + 1), dp2(n + 1);
        // map<ll, set<ll>> dp1, dp2;
        f(a, dp1);
        f(b, dp2);
        dbg(dp1, dp2);
            ...
            for (auto x : y) {
                ...
                if (it == dp2[oth].begin()) {
                    ...
                    if (it != dp2[oth].end()) {
                        csum = *it + x;
                        dbg(abs(sum - 2 * csum));
                        Mindiff = min(Mindiff, abs(sum - 2 * csum));
                    }
                }
                Mindiff = min(Mindiff, here);
                dbg(here, Mindiff);
            }
        }
        it = 0;
...
void solve() {
    dbg(Solution().minimumDifference({76, 8, 45, 20, 74, 84, 28, 1}));
}
#ifdef LOCAL
int32_t main() { solve(); }
#endif