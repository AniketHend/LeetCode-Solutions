...
// solution
class Solution {
  pair<int, int> solve(int n1, int d1, int n2, int d2) {
    int nd = lcm(d1, d2);
    n1 *= nd / d1;
    n2 *= nd / d2;
  ...
  pair<int, int> getCurr(string& exp, int& i) {
    int nmr = 0;
    int dnr = 0;
    int sign = 1;
    int n = sz(exp);
    while (i < n && exp[i] != '/') {
      // cout<<i<<" "<<exp[i]<<" "<<nmr<<endl;
      if (exp[i] == '-')
        sign = -1;
      else if (exp[i] == '+')
        sign = 1;
...
public:
  string fractionAddition(string& exp) {
    int n = sz(exp);
    int i = 0;
    auto fi = getCurr(exp, i);
    int nmr = fi.first;
    int dnr = fi.second;
    while (i < n) {
      auto fr = getCurr(exp, i);
      int cnmr = fr.first;
      int cdnr = fr.second;
      auto sa = solve(nmr, dnr, cnmr, cdnr);
      nmr = sa.first;
      dnr = sa.second;
    }
    if (nmr == 0) {
